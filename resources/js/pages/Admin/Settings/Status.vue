...
const form = useForm({
    title: props.status?.title ?? 'Status',
    content: props.status?.content ?? '',
});

const lastUpdated = computed(() => props.status?.updated_at ?? null);

const submit = () => {
    form.put('/admin/settings/status', {
        preserveScroll: true,
    });
};
</script>
...
.status-preview :deep(h1:first-child) {
    margin-top: 0;
}

.status-preview :deep(h2) {
    font-size: 1.875rem;
    line-height: 2.25rem;
    font-weight: bold;
    margin-top: 1.5rem;
...
.status-preview :deep(h2:first-child) {
    margin-top: 0;
}

.status-preview :deep(h3) {
    font-size: 1.5rem;
    line-height: 2rem;
    font-weight: bold;
    margin-top: 1.25rem;
...
.status-preview :deep(h3:first-child) {
    margin-top: 0;
}

.status-preview :deep(p) {
    margin-bottom: 1rem;
    line-height: 1.75;
}

.status-preview :deep(p:last-child) {
    margin-bottom: 0;
}

.status-preview :deep(ul),
.status-preview :deep(ol) {
    padding-left: 1.5rem;
    margin-top: 0.75rem;
    margin-bottom: 1rem;
}
...
.status-preview :deep(li) {
    margin-bottom: 0.5rem;
}

.status-preview :deep(strong) {
    font-weight: 700;
    color: #fbbf24;
}
</style>
...
<template>
    <div>
        <Head title="Status de l'association" />

        <section class="container mx-auto px-4 py-8">
            <div class="mx-auto max-w-5xl">
                <div class="mb-6">
                    <h1 class="text-3xl md:text-4xl font-bold text-light-blue">Status de l'association</h1>
                    <p class="text-gray-300 mt-2">
                        Rédigez ou mettez à jour les statuts de l'association en utilisant l'éditeur riche. Le contenu est affiché tel quel sur le site public.
                    </p>
                ...
                <div class="grid gap-6 lg:grid-cols-2">
                    <div class="space-y-4">
                        ...
                        <div class="flex gap-3">
                            <Button
                                ...
                                @click="submit"
                            >
                                {{ form.processing ? 'Enregistrement...' : 'Enregistrer les statuts' }}
                            </Button>
                            <Button
                                type="button"
                                variant="outline"
                    ...
                    <div class="rounded-lg border border-light-blue/30 bg-dark/60 p-4">
                        ...
                        <div class="status-preview prose prose-invert max-w-none text-gray-100" v-html="form.content"></div>
                    </div>
                </div>
            </div>
        </section>
